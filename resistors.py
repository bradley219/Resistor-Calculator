...
def resistance_string(value, label='Ω'):
    outval = value
    prefixes = [
            (1000000, 'M'),
            (1000, 'K'),
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--e-series', type=str, default='E24', help='E-series values')
    args = parser.parse_args()

    esv_name = args.e_series.upper()
    series = e_series.get(esv_name)
    if not series:
        print(f"Unknown E series `{args.e_series}'")
        sys.exit(1)

    search(args.amplification, series, args.count, args.min)
